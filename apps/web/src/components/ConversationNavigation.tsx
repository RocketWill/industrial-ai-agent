...
export default function ConversationNavigation({ state, footer }: Props) {
  ...
  return <aside className="conversation-navigation" aria-label="Conversation navigation">
    ...
    <Button className="new-analysis-button" type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>New analysis</Button>
    <nav className="feature-navigation" aria-label="Workbench features">
      ...
      {plannedItems.map((item) => <Button key={item.label} className="feature-item" type="text" icon={item.icon} disabled><span>{item.label}</span><Tag bordered={false}>Planned</Tag></Button>)}
    </nav>
