...
| Industrial workspace shell | Implemented | Ant Design 6.5.1 and Ant Design X 2.9.0 provide a dark-first three-column desktop workbench, a two-column tablet layout, focused mobile Drawers, grouped conversations, Bubble message rendering, a controlled Sender, XMarkdown assistant content, an accessible three-dot processing state, an active-response BorderBeam with a static completed accent, stable latest-message following, a jump-to-latest control, and a current-exchange Production Summary surface. | Full tool timelines, sources, and manufacturing charts are not displayed; evidence is available only for the current exchange. |
