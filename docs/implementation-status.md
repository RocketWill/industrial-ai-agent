...
| Industrial workspace shell | Implemented | Dithered Ant Design theme, desktop sidebar, mobile Drawer, analysis workspace header, explicit synthetic context strip, fixed composer, one main message scrollbar, responsive reading widths, automatic latest-message following, centered scroll-to-bottom control, immediate streaming placeholder, and a compact production evidence summary card. | Tool timelines, sources, and manufacturing charts are not displayed; evidence is available only for the current exchange. |
