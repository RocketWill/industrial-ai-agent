...
| Synchronous assistant response | Implemented | Persist the user message, send complete conversation history to an OpenAI-compatible model, optionally execute one selected production-summary or equipment-status tool call, return optional structured evidence, and persist one successful final assistant response. | No system prompt, retry policy, model discovery, multi-tool turn, or persisted evidence history. |
| Streaming assistant response | Implemented | Send SSE events for the persisted user message, token deltas, completion, safe errors, and supported tool stages (`tool_call_started`, `tool_result`). After a successful tool result, final model text is forwarded as provider token deltas. The browser uses this path for general and supported manufacturing questions. | Client disconnect persistence behavior does not yet have an integration test. |
...
| Industrial workspace shell | Implemented | Ant Design 6.5.1 and Ant Design X 2.9.0 provide a dark-first three-column desktop workbench, a two-column tablet layout, focused mobile Drawers, grouped conversations, Bubble message rendering, a controlled Sender, XMarkdown assistant content, an accessible three-dot processing state, an active-response BorderBeam with a static completed accent, stable latest-message following, a jump-to-latest control, and current-exchange Production Summary or Equipment Status evidence. | Full tool timelines, sources, and manufacturing charts are not displayed; evidence is available only for the current exchange. |
...
The latest local verification on 2026-07-21 produced:

- API: 163 Pytest tests passed and Ruff passed. Package build was not rerun in
...
- Web: 52 Vitest tests passed, TypeScript checking passed, ESLint passed, and
...
JavaScript output is 883.33 kB, or 287.22 kB gzip, while XMarkdown is split into
a 125.66 kB lazy chunk, or 41.52 kB gzip. The initial JavaScript gzip size is
about 17.7 percent above the recorded 244.02 kB baseline and remains within the
