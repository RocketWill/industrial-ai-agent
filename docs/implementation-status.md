...
| Synchronous assistant response | Implemented | Persist the user message, send complete conversation history to an OpenAI-compatible model, optionally execute one selected production-summary, equipment-status, or defect-distribution tool call, return optional structured evidence, and persist one successful final assistant response. | No system prompt, retry policy, model discovery, multi-tool turn, or persisted evidence history. |
...
| Industrial workspace shell | Implemented | Ant Design 6.5.1 and Ant Design X 2.9.0 provide a dark-first three-column desktop workbench, a two-column tablet layout, focused mobile Drawers, grouped conversations, Bubble message rendering, a controlled Sender, XMarkdown assistant content, an accessible three-dot processing state, an active-response BorderBeam with a static completed accent, stable latest-message following, a jump-to-latest control, and current-exchange Production Summary, Equipment Status, or Defect Distribution evidence. | Full tool timelines, sources, and manufacturing charts are not displayed; evidence is available only for the current exchange. |
...
- API: 169 Pytest tests passed and Ruff passed. Package build was not rerun in
...
- Web: 56 Vitest tests passed, TypeScript checking passed, ESLint passed, and
...
JavaScript output is 900.59 kB, or 292.60 kB gzip, while XMarkdown is split into
a 125.66 kB lazy chunk, or 41.52 kB gzip. The initial JavaScript gzip size is
about 19.9 percent above the recorded 244.02 kB baseline and remains within the
