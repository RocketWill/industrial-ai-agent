...
| Manufacturing domain | In Progress | Immutable Equipment, Production Lot, Inspection Record, Defect Count, Alarm Event, Time Range, and Production Summary types; deterministic yield and defect aggregation; overlapping alarm selection; explicit empty-result behavior; and one fictional AOI dataset. | No API, database persistence, graph tool, equipment-status semantics, throughput calculation, or causal inference. |
| Production summary tool | In Progress | Typed `get_production_summary` request/result boundary filters the synthetic AOI dataset and delegates numeric work to the manufacturing domain. | Not connected to LangGraph, HTTP, or UI; no live data or model-generated interpretation. |
| LangGraph orchestration | Implemented | The synchronous path invokes a compiled typed graph. The SSE runner reuses its state, context-loading, and persistence boundaries while emitting token events directly. | No persisted runs or node events, conditional routing, tool nodes, retries, checkpoints, resume behavior, or graph visualization. |
...
The latest full local verification on 2026-07-13 produced:

- API: 135 Pytest tests passed, Ruff passed, and the source and wheel builds
  completed.
...
show the intended workflow. The new domain module can calculate summaries from
its fictional dataset, but it is not connected to the API, graph, or UI.
Model answers about yield, alarms, defects, or equipment status therefore
remain ungrounded model output and must not be presented as verified domain
results.
