...
final `assistant_message` event carries the canonical snapshot. A synchronous
Combined partial-failure test retains succeeded manufacturing and failed
documents (`error_code: TOOL_UNAVAILABLE`). These tests reuse shared
`persist_response`; no production code changed. This focused seam does not
complete Slice 3's atomic-persistence acceptance boundary. Runtime acceptance
for the remaining single-route paths and the combined SSE path, rollback,
cancellation, and client disconnect remain open. The read adapter, service/API
wiring, historical reload responses and UI, and Model Working Notes are not
yet implemented.
