...
| Production summary tool | In Progress | Typed `get_production_summary` request/result boundary filters the synthetic AOI dataset, delegates numeric work to the manufacturing domain, participates in the synchronous grounded-answer flow, and resolves missing arguments from supported workspace context. | Structured evidence is not exposed through HTTP or the UI; custom or unrecognized time-range labels require clarification. |
